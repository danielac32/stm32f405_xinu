...
MINIRV32_DECORATE int32_t MiniRV32IMAStep( struct MiniRV32IMAState * state, uint8_t * image, uint32_t vProcAddress, uint32_t elapsedUs, int count )
{
	...
	//if( CSR( extraflags ) & 4 ){
	//	printf("If WFI, don't run processor.\n");
	//	return 1;
	//}
	...
	if((  CSR( mip ) & (1<<3) ) && ( CSR( mie ) & (1<<3)  ) && ( CSR( mstatus ) & 0x8 ) ){
		trap = 0x80000003;
		pc -= 4;
		...
		for( int icount = 0; icount < count; icount++ )
		{
			...
			if( ofs_pc >= MINI_RV32_RAM_SIZE )
			{
				...
				//printf("opcode %08x %08x %d %d %d\n",ir, ofs_pc,cycle,CSR( cyclel ),CSR( cycleh ));
				...
				switch( ir & 0x7f )
				{
						...
						if( addy >= MINI_RV32_RAM_SIZE-3 )
						{
							...
							if( addy >= 0x10000000 && addy < 0x12000000 )
							{
								...
								if( addy == 0x11004004 ) //CLNT
									...
									CSR( timermatchl ) = rs2;
								else if( addy == 0x11100001 ) //SYSCON (reboot, poweroff, etc.)
								{
									SETCSR( pc, pc + 4 );
					...
					case 0x33: // Op           0b0110011
					{
						...
						if( is_reg && ( ir & 0x02000000 ) )
						{
							switch( (ir>>12)&7 ) //0x02000000 = RV32M
							{
	                            ...
	                            // If compiling on a system that doesn't natively, or via libgcc support 64-bit math.
								...
								case 4: if( rs2 == 0 ) rval = -1; else rval = ((int32_t)rs1 == INT_MIN && (int32_t)rs2 == -1) ? rs1 : ((int32_t)rs1 / (int32_t)rs2); break; // DIV
								case 5: if( rs2 == 0 ) rval = 0xffffffff; else rval = rs1 / rs2; break; // DIVU
								case 6: if( rs2 == 0 ) rval = rs1; else rval = ((int32_t)rs1 == INT_MIN && (int32_t)rs2 == -1) ? 0 : ((uint32_t)((int32_t)rs1 % (int32_t)rs2)); break; // REM
					...
					case 0x73: // Zifencei+Zicsr  (0b1110011)
					{
						...
						if( (microop & 3) ) // It's a Zicsr function.
						{
							...
							case 0x403: SETCSR( mtvec2, writeval ); break;
