...
int sys_putc(int des,char c){
    return __syscall(XINU_PUTC,des,c);
}
...
void *sys_js0n(void *frame){
    return (void *)__syscall(XINU_JSON,frame);
}
...
uint32 len_fifo_usb(){
        return __syscall(XINU_GET_LEN);
}
char *fifo_usb(){
       return (char *)__syscall(XINU_GETS);
}
