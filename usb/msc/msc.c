...
static usbd_respond msc_getdesc(usbd_ctlreq *req, void **address, uint16_t *length)
{
    ...
    case USB_DTYPE_STRING:
        ...
        {
            kprintf("error msc_getdesc\n");
            return usbd_fail;
        }
        break;
    default:
        kprintf("error msc_getdesc 2\n");
        return usbd_fail;
    }
    if (len == 0)
    {
...
static void bot_rx(usbd_device *dev)
{
    ...
    case bot_idle:
        ...
        case SCSI_WRITE_10:
            {
                cdb_write_10_t *cdb;
                sd_info_t *sd_info;//sd_info_t *sd_info;
...
void check_msc(){
    if (!hw_get_pin(GPIOx(GPIO_A),0)){
        ...
        //kprintf("size card: %d sector: %d\n",sd_info->card_size*sd_info->sect_size,sd_info->sect_size);
        usbd_hw_init(&udev);
